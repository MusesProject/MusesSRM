<%-- 
    Document   : violations_user
    Created on : 23-oct-2015, 13:01:03
    Author     : paloma
--%>
...
<html>
    <head>
        ...
        <c:if test="${empty param.user_id}">
            <c:redirect url="p_violations.jsp" >
                <c:param name="errMsg" value="Please select a valid user." />
            </c:redirect>
        </c:if>
        
        <sql:query dataSource="${snapshot}" var="graphResults">
            SELECT count(*) as e, date(detection) as d FROM security_violation WHERE user_id='${param.user_id}' GROUP BY date(detection);
        </sql:query>
    ...
    <body>
        ...
        <c:if test="${totalCount == 0 }">
            <c:redirect url="p_violations.jsp" >
                <c:param name="errMsg" value="No security violations registered for that user." />
            </c:redirect>
        </c:if>
        ...
        <div class="ui purple animated button" align="right" onclick="location.href = 'p_violations.jsp'">
            <div class="visible content">Back to user picking</div>
